...
export class SighashAllSigning {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
...
export function SerializeSighashAllSigning(value) {
  const buffers = [];
  buffers.push(SerializeScript(value.signing_script));
  return serializeTable(buffers);
}

export class LuaSigning {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
...
export function SerializeLuaSigning(value) {
  const buffers = [];
...
export function SerializeSigningMethods(value) {
  switch (value.type) {
...
export function SerializeUnsignedTransaction(value) {
  const buffers = [];
