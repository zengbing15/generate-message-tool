(function (global, factory) {
  ...
  function SerializeSighashAllSigning(value) {
    const buffers = [];
    buffers.push(SerializeScript(value.signing_script));
    return serializeTable(buffers);
  }
  ...
  function SerializeLuaSigning(value) {
    const buffers = [];
  ...
  class SigningMethods {
    ...
    validate(compatible = false) {
      ...
      switch (t) {
      case 0:
        new SighashAllSigning(this.view.buffer.slice(4), { validate: false }).validate();
        break;
      case 1:
        new LuaSigning(this.view.buffer.slice(4), { validate: false }).validate();
        break;
      default:
    ...
    value() {
      ...
      switch (t) {
      case 0:
        return new SighashAllSigning(this.view.buffer.slice(4), { validate: false });
      case 1:
        return new LuaSigning(this.view.buffer.slice(4), { validate: false });
      default:
        throw new Error(`Invalid type: ${t}`);
      }
    }
  ...
  function SerializeSigningMethods(value) {
    switch (value.type) {
    case "SighashAllSigning":
      {
        const itemBuffer = SerializeSighashAllSigning(value.value);
    ...
    case "LuaSigning":
      {
        const itemBuffer = SerializeLuaSigning(value.value);
        const array = new Uint8Array(4 + itemBuffer.byteLength);
        const view = new DataView(array.buffer);
  ...
  class UnsignedTransaction {
    ...
    validate(compatible = false) {
      const offsets = verifyAndExtractOffsets(this.view, 0, true);
      new SigningMethods(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
      new Transaction(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
      new TransactionVec(this.view.buffer.slice(offsets[2], offsets[3]), { validate: false }).validate();
      new TransactionVec(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
      new HeaderVec(this.view.buffer.slice(offsets[4], offsets[5]), { validate: false }).validate();
    }
    ...
    getSigningMethod() {
      ...
      const offset = this.view.getUint32(start, true);
      const offset_end = this.view.getUint32(start + 4, true);
      return new SigningMethods(this.view.buffer.slice(offset, offset_end), { validate: false });
    }
  ...
  exports.SerializeLuaSigning = SerializeLuaSigning;
  ...
  exports.SerializeSighashAllSigning = SerializeSighashAllSigning;
  exports.SerializeSigningMethods = SerializeSigningMethods;
  ...
  exports.SighashAllSigning = SighashAllSigning;
